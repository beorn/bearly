...
class DotzReporter implements Reporter {
  ...
  private async startStreaming() {
    ...
    const { render, createTerm } = await import("silvery")
    ...
    this.app = stack.use(
      await render(
        <Report store={this.store} options={this.options} console={this.patchedConsole ?? undefined} />,
        this.term,
        {
          // mode: "inline",
...
async function printSummary(store: TestStore, options: Options) {
  const { renderStatic } = await import("silvery")
