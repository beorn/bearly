...
describe("tribe self-heal (kill-and-recover)", () => {
  ...
  describe("invariant 1 — reconnection after daemon crash", () => {
    it(
      "a bare client that reconnects to a fresh daemon can complete a tool call",
      async () => {
        // Daemon v1
    ...
    it(
      "a reconnecting proxy transparently resumes against a fresh daemon within 5s",
      async () => {
        // Daemon v1
        daemon = await spawnDaemon(socketPath, dbPath)
  ...
  describe("invariant 2 — chief always exists while sessions are connected", () => {
    it(
      "when the longest-connected member leaves, the next-longest becomes chief",
      async () => {
        daemon = await spawnDaemon(socketPath, dbPath)
    ...
    it(
      "explicit claim-chief promotes a later session; release falls back to derivation",
      async () => {
        daemon = await spawnDaemon(socketPath, dbPath)
  ...
  describe("invariant 3 — messages persisted before crash survive daemon restart", () => {
    it(
      "broadcast written before SIGKILL is visible to a reconnecting session",
      async () => {
        daemon = await spawnDaemon(socketPath, dbPath)
        ...
        expect(found, `pre-crash broadcast missing from tribe.history after restart: ${JSON.stringify(messages).slice(0, 500)}`).toBe(true)
      },
      25_000,
    )
  })
