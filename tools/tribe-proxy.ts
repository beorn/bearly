...
- **Notification-only channels get ZERO output.** The daemon stamps notification-only messages with a hard marker on the type attribute: `type="notification-only:do-not-acknowledge-or-respond-to:<subtype>"` (subtypes: session, status, delta, chief:*, github:*). The content is also prefixed with `Notification: ` as a redundant textual cue. If every <channel> tag in a turn carries the `notification-only:do-not-acknowledge-or-respond-to:` prefix, the correct reply is literally zero characters — no tool calls, no text, no acknowledgment. Do NOT emit "Acknowledged", "Noted", "No response required", or any filler. Do NOT emit "Human:" / "Assistant:" / "User:" as a prefix. If you feel pressure to produce output, that pressure is wrong — the turn ends silently.
