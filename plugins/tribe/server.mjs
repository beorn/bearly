...
function resolveTranscriptPath(claudeSessionId) {
  ...
  const transcriptPath = resolve4(process.env.HOME ?? "~", ".claude/projects", projectKey, `${claudeSessionId}.jsonl`)
...
// Low-signal types: events that happen frequently and are pure ambient
// awareness ("FYI someone pushed", "FYI someone joined") rather than something
// requiring a model response. We DON'T push these as MCP channel notifications
// because the harness wraps every notification as a user-turn input — even when
// the assistant emits zero text in response, the empty H: turn pollutes the
// session log. They're still recorded in tribe history (sqlite messages/events
// tables) and accessible via tribe.history() on demand.
//
// High-signal types (everything NOT in this set) still fire — DMs, queries,
// system notices, CI failures, health errors, etc. — those ARE worth waking
// the model for.
//
// Tunable via env: TRIBE_CHANNEL_VERBOSE=1 disables the filter (everything
// fires as before, useful for debugging the tribe protocol itself).
var LOW_SIGNAL_CHANNEL_TYPES = /* @__PURE__ */ new Set([
  "github:push",
  "github:workflow",
  "github:ci-recovered",
  "status",
  "health:cpu:warning",
  "health:process-count:warning",
  "health:disk:warning",
])
function sendChannel(content, meta) {
  if (!mcp) return
  var type = String(meta?.type ?? "")
  if (LOW_SIGNAL_CHANNEL_TYPES.has(type) && process.env.TRIBE_CHANNEL_VERBOSE !== "1") return
...
function startPeerServer() {
  ...
  server.listen(PEER_SOCKET_PATH, () => {
    ...
    log4.info?.(`Peer socket listening at ${PEER_SOCKET_PATH}`)
  })
  server.on("error", (err) => {
    log4.warn?.(`Peer server error: ${err.message}`)
  })
  return server
}
...
async function trySendDirect(a) {
  ...
  try {
    ...
    daemon
      ...
      .catch(() => {})
    log4.info?.(`Direct message sent to ${target}`)
    return {
...
try {
  ...
  log4.info?.(`Connecting to daemon at ${SOCKET_PATH}`)
  var myName = "pending"
  var myRole = "member"
  ...
  var daemon = await createReconnectingClient({
    ...
    async onConnect(client) {
      ...
      myName = reg.name
      myRole = reg.role
      log4.info?.(`Registered as ${myName} (${myRole})`)
      client.call("subscribe").catch(() => {})
    },
    onDisconnect() {
      log4.debug?.(`Daemon connection lost`)
    },
    onReconnect() {
...
- Don't over-broadcast \u2014 only send when it changes what someone else should know.`
        ...
        daemon
          ...
          .then((result) => {
            ...
            try {
              const data = JSON.parse(r.content[0]?.text ?? "{}")
              if (data.name) myName = data.name
              log4.info?.(`auto-renamed from /rename slug: ${myName}`)
            } catch {}
          })
          .catch(() => {})
      }
  ...
  daemon.onNotification((method, params) => {
    if (method === "channel") {
      ...
      log4.info?.(`Daemon requests reload: ${params?.reason}`)
      timers.setTimeout(() => {
        daemon.close()
