...
// packages/daemon-spine/src/rpc.ts
...
// packages/daemon-spine/src/parser.ts
var log = createLogger("daemon-spine:parser");
...
// packages/daemon-spine/src/client.ts
var log2 = createLogger("daemon-spine:client");
...
// packages/daemon-spine/src/paths.ts
...
// packages/daemon-spine/src/composition/scope.ts
...
function createReconnectingClient2(opts) {
  ...
  return createReconnectingClient(spineOpts);
}
...
var TOOLS_LIST = [
  ...
  {
    name: "tribe.mode",
    description: "Set the per-session focus mode. `focus` = only direct DMs and threshold-escalated alerts reach the channel; `normal` = kind-based default; `ambient` = everything to channel (escape hatch). Persisted across reconnects.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "tribe.snooze",
    description: "Time-bounded silence on channel events for this session. duration_sec=0 cancels any active snooze. Optional `kinds` is a list of plugin_kind globs (e.g. ['github:*']) to silence; omit to silence everything. Direct messages always bypass snooze.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "tribe.dismiss",
    description: "Acknowledge an actionable event without replying. Inserts a row into the dismissals audit table (used as classifier-training signal \u2014 high dismiss rate on a kind suggests it should be reclassified ambient). Optional `reason` describes why.",
    inputSchema: {
      type: "object",
      properties: {
...
let __stack = [];
try {
  var log5 = createLogger("tribe:proxy");
