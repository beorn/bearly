...
- `lore.inject_delta(prompt, sessionId?, limit?, ttlTurns?)` — new MCP
...
- No behaviour change when daemon is reachable — same dedup semantics
  ...
  prompt.
- `LORE_NO_DAEMON=1` still forces the library path; integration tests
  exercise both.
...
2 new integration tests (`tests/lore/daemon.test.ts`): inject_delta
...
one-sentence focus + loose-ends list. Exposed via new `lore.session_state`
MCP tool and surfaced in `lore.workspace_state` (new `focusSummary`,
`looseEnds`, `summaryModel`, `summaryUpdatedAt` fields).

### Added

- `tools/lib/lore/summarizer.ts` — pure `summarizeTail(tail, {mode})` using
  existing `getCheapModel`/`queryModel`. Haiku 4.5 by default; Ollama when
  `LORE_SUMMARIZER_MODEL=local`. Strict-ish JSON parser strips code fences
...
- Daemon summarizer coroutine — runs every `LORE_SUMMARY_POLL_MS`
  ...
  Skipped entirely when `LORE_SUMMARIZER_MODEL=off` (default).
- `lore.session_state(sessionId)` MCP tool + RPC — returns focus +
  summary + tail for one session. Errors on unknown sessionId.
- `lore.workspace_state` rows now include `focusSummary`, `looseEnds`,
...
- `LORE_SUMMARIZER_MODEL=off` (default) — no LLM calls, no cost.
- `LORE_SUMMARIZER_MODEL=haiku` — Claude Haiku 4.5 (~\$0.00001 / summary).
- `LORE_SUMMARIZER_MODEL=local` — Ollama when available.

### Tests

7 new summarizer unit tests (`tests/lore/summarizer.test.ts`): JSON
...
poller. `lore.current_brief` reads from the cache when fresh.
...
- **`lore.workspace_state` MCP tool** — cross-session snapshot; each entry
  combines session metadata with cached focus. Used by `bear sessions` /
  `bear workspace` CLI commands.
- **`lore.current_brief` cache fast-path** — when the caller supplies a
...
- `lore-daemon --focus-poll-ms <ms>` and `LORE_FOCUS_POLL_MS` env var
  expose the poll interval (tests use 200 ms, default 60 s).

### Tests

5 new integration + unit tests (`tests/lore/focus.test.ts`): pure
...
server now bridges Claude Code to a persistent `lore-daemon` via a Unix
socket, keeping the recall library warm across calls.

### Added

- **lore-daemon** — long-lived process at `$XDG_RUNTIME_DIR/lore.sock` with
  SQLite WAL at `~/.local/share/lore/lore.db`. 30 min idle timeout. Fresh
  spawn on first connection via `connectOrStart` (modeled on
  `@bearly/tribe` daemon lifecycle). Handles `lore.ask`, `lore.current_brief`,
  `lore.plan_only`, plus session registry (`lore.session_register`,
  `lore.session_heartbeat`, `lore.sessions_list`), `lore.hello`, `lore.status`.
...
- **`LORE_NO_DAEMON=1`** — opt-out for debugging / deterministic tests.
- **`LORE_LOG=1`** — enable recall library + connect-failure stderr output.
...
- Bear database (`~/.local/share/lore/lore.db`) is separate from tribe's
...
- `lore.ask` MCP tool — wraps `recallAgent()`, exposes 2-round planner +
  fanout + synthesis as a structured Claude Code tool.
- `lore.current_brief` MCP tool — wraps `getCurrentSessionContext()`.
- `lore.plan_only` MCP tool — round-1 planner only, no fanout/synth.
